...
exports.clearState = () => {
    const f = `${CONFIG_DIR}/state.json.cached`;
    if (fs.existsSync(f)) {
        fs.unlinkSync(f)
    }
}
...
exports.clearCachedFiles = () => {
    const files = glob.sync(`${CONFIG_DIR}/*.cached`);
    files.map(f => {
        fs.unlinkSync(f);
    });
}
...
exports.getCachedAuthToken = () => {
    return JSON.parse(fs.readFileSync(`${CONFIG_DIR}/auth.json.cached`, 'utf8')).access_token;
}

exports.getCachedAuthUser = () => {
    return JSON.parse(fs.readFileSync(`${CONFIG_DIR}/auth.json.cached`, 'utf8')).username;
}

exports.clearAuthToken = () => {
    const f = `${CONFIG_DIR}/auth.json.cached`;
    if (fs.existsSync(f)) {
        fs.unlinkSync(f)
    }
}
