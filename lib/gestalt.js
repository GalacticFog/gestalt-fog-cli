...
exports.fetchWorkspaces = (fqonList) => {
    return fetchTypeFromOrgs("workspaces", fqonList);
}
...
exports.fetchOrgEnvironments = (fqonList) => {
    return fetchTypeFromOrgs("environments", fqonList);
}

exports.fetchOrgApis = (fqonList) => {
    return fetchTypeFromOrgs("apis", fqonList);
}

exports.fetchOrgLambdas = (fqonList) => {
    return fetchTypeFromOrgs("lambdas", fqonList);
}
...
exports.fetchApiEndpoints = (apiList) => {
    // https://test.galacticfog.com/meta/engineering.eric-test/apis/986137e6-ebdb-46f0-9ac5-d1e692ab8129/apiendpoints?expand=true
    const state = getGestaltState();
    if (!state.org) throw Error("No Org in current context");
    if (!state.org.fqon) throw Error("No FQON in current context");
    ...
    let eps = apiList.map(api => {
        const res = meta_GET(`/${state.org.fqon}/apis/${api}/apiendpoints?expand=true`)
        return res;
    });
...
function fetchTypeFromOrgs(type, fqonList) {
